...
export const ModernPlayer: React.FC<ModernPlayerProps> = ({ currentVideo, onPrevious, onNext }) => {
  ...
  return (
    <div
      className={`fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-gray-900/90 backdrop-blur-2xl shadow-lg transition-all duration-300 h-20 border-t border-pink-100/50 dark:border-pink-900/30`}
    >
      <div className="flex items-center justify-between h-full px-6 max-w-[1920px] mx-auto">
        ...
        <div className="flex items-center space-x-3 w-1/4 min-w-[220px]">
          <div className="relative group">
            ...
            <div className="absolute inset-0 rounded-xl bg-pink-400/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium text-gray-900 dark:text-white truncate group-hover:text-pink-500 dark:group-hover:text-pink-400 transition-colors duration-150">
        ...
        <div className="flex flex-col items-center w-2/4 max-w-[500px] px-4">
          ...
          <div className="flex items-center justify-center space-x-6 mb-2">
            ...
            <button
              onClick={togglePlay}
              className="text-gray-800 dark:text-gray-200 hover:text-pink-500 dark:hover:text-pink-400 transition-all duration-150 
              p-2 rounded-full hover:bg-pink-50 dark:hover:bg-pink-900/20 relative group"
            >
          ...
          <div className="flex items-center space-x-2 w-full">
            <span className="text-[10px] font-medium text-gray-500 dark:text-gray-400 w-8 text-right select-none">
            ...
            <div className="flex-1 group relative h-1.5">
              ...
              />
              <div className="absolute h-2 w-2 bg-pink-500 dark:bg-pink-400 rounded-full shadow-sm top-1/2 -mt-1 -ml-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150"
                style={{ left: `${(currentTime / duration) * 100}%` }}
              />
            </div>
            <span className="text-[10px] font-medium text-gray-500 dark:text-gray-400 w-8 select-none">
              {formatTime(duration)}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end space-x-3 w-1/4 min-w-[180px]">
          <button
            onClick={toggleMute}
            className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors duration-150 p-1.5 rounded-full hover:bg-pink-50 dark:hover:bg-pink-900/20"
          >
            {isMuted || volume === 0 ? (
              <svg
                xmlns="http://www.w3.org/2000/svg"
